...
def test_first_level_nested_projects_are_cleaned_as_own_roots(
    repository: Path,
) -> None:
    sub = repository / "box-gui"
    sub.mkdir()
    (sub / "pyproject.toml").touch()
    venv_python = sub / ".venv" / "bin" / "python"
    venv_python.parent.mkdir(parents=True)
    venv_python.write_bytes(b"interpreter")
    cache = sub / ".pytest_cache"
    cache.mkdir()
    deep = bytecode(sub / "vendor" / "__pycache__" / "deep.pyc")
    (sub / "vendor" / "pyproject.toml").touch()
    targets = cleaner.collect_targets(repository)
    assert sub / ".venv" in targets["venvs"]
    assert cache in targets["caches"]
    assert cleaner.remove(targets)
    assert not (sub / ".venv").exists()
    assert not cache.exists()
    assert (sub / "pyproject.toml").exists()
    assert deep.exists()
