...
Releases are tag-driven. The builder computes the `year.month.commit-count` tag from git history (same count as the version-standard script, aligned with the `chore(release)` commits), creates the annotated tag at HEAD, and then verifies `git describe --exact-match` plus the tag/HEAD SHAs before staging anything; any mismatch fails closed. Local test builds pass `--no-create-tag` to embed without creating or verifying tags. The tag is baked into `AppRun` (`BOX_RPG_MAKER_APPIMAGE_TAG`) and a generated `box_gui/appimage_tag.txt` inside the payload (never committed).
