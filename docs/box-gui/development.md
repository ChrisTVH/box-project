...
- `src/box_gui/gtk/threads.py`: backend-free `run_in_thread` with daemon thread plus `GLib.idle_add` marshaling, re-exported by `workers.py` for the backend-bound flows. `src/box_gui/gtk/workers.py`: `ProgressReporter` forwarding `(completed, total)` to the main loop, and thin `run_inspect`, `run_launch`, and `run_diagnose` wrappers. The backend setup page drives detection and install through `threads` so startup survives a missing backend.
...
- `src/box_gui/core/game_icon.py`: game icon discovery without `Gtk`/`Adw` (`GdkPixbuf` decoding is allowed). Lists `.exe` files through `list_root_files`, converts `.ico` to PNG with `icoextract` and `GdkPixbuf`, and builds the app-owned cache path. Executables are only read, never run. Failures return `False` instead of raising. `icoextract` is optional: without it extraction reports unavailable (setup-page row, Icon-row warning with install hint; exe picks and extraction failures stay silent) and icons fall back to the engine default.
...
`tools/build_appimage.py` (with the `tools/appimage_tag.py` fallback reader) builds `box-rpg-maker.appimage` as a Python-pure, CI-reusable step with no shell scripts. The artifact packs only the frontend: the `box_gui` sources with the in-package `locale/`, plus the `res/` icons, the desktop entry, and the app icon. The `box-rpg` backend and the Python interpreter are never bundled; the generated `AppRun` starts on `/usr/bin/python3` and, when it is older than 3.14, re-executes the first Python 3.14+ with GTK bindings from `$BOX_RPG_MAKER_PYTHON`, `python3` on `PATH`, then `/usr/local/bin/python3`, `/usr/local/bin/python3.14`, `/usr/bin/python3.14`, so install the backend first (`./install.py --install --target cli`) for the direct-to-library path. A missing or stale backend is owned by `BackendSetupPage`, never by this launcher: the first run without one opens the in-app setup instead of refusing to start. On externally managed Pythons the one-click setup retries once automatically with `--break-system-packages` after pip refuses with an externally-managed-environment error, streaming both attempts verbatim; `--yes` alone never implies that consent. Before cloning, a read-only probe reports user-site ancestors violating install.py's ownership rule (wrong owner or group/other write bits) with their exact paths instead of failing after the build.
...
Releases are tag-driven. The builder computes the `year.month.commit-count` tag from git history via `tools/versioning.py` (same source as the version-standard script), creates the annotated tag at HEAD, and then verifies `git describe --exact-match` plus the tag/HEAD SHAs before staging anything; any mismatch fails closed. Local test builds pass `--no-create-tag` to embed without creating or verifying tags. The tag is baked into `AppRun` (`BOX_RPG_MAKER_APPIMAGE_TAG`) and a generated `box_gui/appimage_tag.txt` inside the payload (never committed).
