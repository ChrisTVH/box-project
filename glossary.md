...
| --- | --- | --- |
| game root | raíz del juego | Keep command selectors unchanged. |
| runtime | entorno de ejecución | Keep `runtime` command unchanged. |
...
| cleanup | limpieza | Keep `cleanup` command unchanged. |
...
| plugin registry | registro de complementos | |
| manifest | manifiesto | |
| plugin | complemento | |
...
| ownership | propiedad | |
| web game | juego web | |
| completion | autocompletado | En mensajes sobre completados del shell. |
| download | descarga | |
